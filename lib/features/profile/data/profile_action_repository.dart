...
class ProfileActionRepository {
  ProfileActionRepository({required Bluesky bluesky}) : _bluesky = bluesky;

  final Bluesky _bluesky;

  Future<String> followActor({required String did}) async {
    final response = await _bluesky.graph.follow.create(subject: did, createdAt: DateTime.now());
  ...
  Future<void> unfollowActor({required String followUri}) async {
    ...
    await _bluesky.graph.follow.delete(rkey: rkey);
  }
  ...
  Future<String> blockActor({required String did}) async {
    final response = await _bluesky.graph.block.create(subject: did, createdAt: DateTime.now());
  ...
  Future<void> unblockActor({required String blockUri}) async {
    ...
    await _bluesky.graph.block.delete(rkey: rkey);
  }
